...
class DashboardCard {
  ...
  _createMarkup() {
    ...
    this.container.insertAdjacentElement('beforeend', `
      <div class="dashboard__card dashboard__card--${id}">
        <article class="tracking-card">
          ...
          <div class="tracking-card__body">
            ...
            <div class="tracking-card__prev-period">Last ${DashboardCard.PERIODS[this.view]} - ${previous}hrs</div>
          </div>
        </article>
      </div>
    `);

    this.time = this.container.querySelector(`.dashboard__card--${id} .tracking-card__time`);
    this.prev = this.container.querySelector(`.dashboard__card--${id} .tracking-card__prev-period`);
  }
  ...
  changeView(view) {
    ...
    this.prev.innerText= `Last ${DashboardCard.PERIODS[this.view]} - ${previous}hrs`;
...
document.addEventListener('DOMContentLoaded', () => {
  getDashboardData()
    .then(data => {
      const activities = data.map(activity => new DashboardCard(activity));

      const selectors = document.querySelector('.view-selector__item');
